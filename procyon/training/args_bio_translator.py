...
class ModelArgs:
    ...
    protein_struct_embeddings_path: str = field(
        default = os.path.join(DATA_DIR, "generated_data/node_embeddings/protein/indexed_total_embeddings.pt"),
        metadata = {
    ...
    bio_encoder_feature: str = field(
        default='seqs,network,description,expression'
    )
    bio_encoder_hidden_dim: int = field(
        default=1000
    )
    
    bio_seq_input_nc: int = field(
        4
    )
    
    bio_seq_in_nc: int = field(
        512
    )
    
    bio_seq_max_kernels: int = field(
        129
    )
    
    bio_seq_length: int = field(
        2000
    )
    
    bio_network_dim: int = field(
        800
    )
    
    bio_description_dim: int = field(
        768
    )
    bio_text_dim: int = field(
        768
    )
    
    bio_feature_encoder_ckpt: str = field(
        'tmp/bio_encoder.pth'
    )
    
    bio_text_encoder:str = field(
        'microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract-fulltext'
    )
    
    bio_text_output_way: str = field(
        'pooler'
    )
...
def update_data_dir_args(model_args: ModelArgs):
