...
class EvalArgs:
    ...
    model_args_from_checkpoint: str = field(
        default="",
        metadata={
            "help": "Load ModelArgs from a TxPLM checkpoint. Overrides YAML or CLI, "
    ...
    data_args_from_checkpoint: str = field(
        default="",
        metadata={
            "help": "Load DataArgs from a TxPLM checkpoint. Overrides YAML or CLI, "
                    "this can be important for data loading to match what the saved "
                    "data expects.",
        },
    )
    ...
    caption_max_len: int = field(
        default = 64,
        metadata = {
            "help": "Maximum length (in tokens) for caption generation."
        }
    )