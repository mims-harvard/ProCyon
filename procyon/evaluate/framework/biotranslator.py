...
class TextEncoder(nn.Module):
    ...
    def forward(self, text):
        input_ids = self.tokenizer(text, max_length=self.maxlen, truncation=True, padding='max_length',
                                return_tensors='pt').to(self.text.device).input_ids
...
class BioTranslator(nn.Module):
    ...
    def forward(self, input_seq, input_description, input_vector, texts):
        ...
        # compute logits
        return data_encodings
        # print(data_encodings.shape, text_encodings.shape)
...
class BioTranslatorRetrievalEval(AbstractRetrievalModel):
    def __init__(
        ...
        self.protein_tokenizer = Alphabet.from_architecture(model_args.protein_tokenizer_name)
        self.decode_mapping = {}
    ...
    def get_predictions(
        ...
        if not isinstance(query_loader.dataset, AASeqTextUnifiedDataset):
            raise ValueError(f"biotranslator expected query dataset to be AASeqTextUnifiedDataset, "
        ...
        for protein_ids in tqdm(target_loader):
            ...
            seqs = [query_loader.collate_fn.aaseq_sequences[x] for x in batch_target_ids]
